...
from vllm.entrypoints.qwen.utils.schema import ASSISTANT, DEFAULT_SYSTEM_MESSAGE, FUNCTION, SYSTEM, USER, ContentItem, \
    Message, \
    TOOL, FunctionCall, ROLE, CONTENT
from vllm.entrypoints.openai.protocol import ToolCall, ChatCompletionMessageParam, ChatMessage, ChatCompletionToolsParam
...
def _truncate_input_messages_roughly(messages: List[Message], max_tokens: int) -> List[
    Message]:
    sys_msg = messages[0]
    assert sys_msg.role == SYSTEM  # The default system is prepended if none exists
    if len([m for m in messages if m.role == SYSTEM]) >= 2:
        raise 'The input messages must contain no more than one system message.  And the system message, if exists, must be the first message.'
    ...
    for m in messages[1:]:
        if m.role == USER:
            ...
            if turns:
                ...
                raise 'The input messages (excluding the system message) must start with a user message.'

    def _count_tokens(msg: Message) -> int:
        token_ids = tokenizer.tokenize(extract_text_from_message(msg, add_upload_info=True))
    ...
    if len(truncated) < 2:  # one system message + one or more user messages
        raise 'At least one user message should be provided.'
    if token_cnt > max_tokens:
        raise (f'The input messages exceed the maximum context length ({max_tokens} tokens) after '
...
def encode_to_qwen_message(messages: List[Union[Dict, ChatCompletionMessageParam]]) -> List[Message]:
    qwen_messages = []
    tool_id_name = {}
    ...
    for msg in messages:
        ...
        if msg[ROLE] == ASSISTANT:
            ...
            if "tool_calls" in msg and msg["tool_calls"]:
                for tool in msg["tool_calls"]:
                    qwen_messages.append(
                        Message(role=msg[ROLE], content="", function_call=FunctionCall(name=tool["function"]["name"],
        ...
        if msg[ROLE] == FUNCTION or msg[ROLE] == TOOL:
            name = msg.get("name", "")
            if not name:
                name = tool_id_name[msg["tool_call_id"]]
            qwen_messages.append(
                Message(role=FUNCTION, name=name, content=msg[CONTENT])
            )
    return qwen_messages


def std_qwen_to_openai_message(messages: List[Message]) -> List[ChatCompletionMessageParam]:
    new_messages = []
    for msg in messages:
        if msg.role == SYSTEM:
            new_messages.append(ChatCompletionSystemMessageParam(role=SYSTEM, content=msg.content))
        if msg.role == USER:
            new_messages.append(ChatCompletionUserMessageParam(role=USER, content=msg.content))
        if msg.role == ASSISTANT:
            new_messages.append(ChatCompletionAssistantMessageParam(role=ASSISTANT, content=msg.content))
    return new_messages


def qwen_to_openai_response_message(messages: list[Message]):
    new_messages = ChatMessage(role=ASSISTANT, content="", )
    fn_id = 1
    for msg in messages:
        assert msg.role == ASSISTANT, f"model return data role mast be assistant: {messages}"
        if msg.content:
            if new_messages.content:
                new_messages.content += "\n".join([t.text for t in msg.content])
            else:
                new_messages.content = "\n".join([t.text for t in msg.content])
        if msg.function_call:
            en_id = encrypt_fn_id(fn_id)
            new_messages.tool_calls.append(function_to_tool_call(msg.function_call, en_id))
            fn_id += 1
    return new_messages


def tool_calls_to_functions(tools: List[ChatCompletionToolsParam]) -> List[Dict]:
    functions = []
    for tool in tools:
...
def function_to_tool_call(function_call, fn_id: str):
    return ToolCall(
        function=Function(**function_call.model_dump()),
        type="function",
        id=fn_id
    )


def fnresult_to_tool_call_result(fn_result, fn_id: str):
    return ChatCompletionToolMessageParam(
        tool_call_id=fn_id,
        role=TOOL,
        content=fn_result.content
    )


def encrypt_fn_id(fn_id: int):
    import base64
    fn_id_str = f"000000{fn_id}"[-7:]
    fn_id_str = "qwen_model_" + fn_id_str
    encrypted = base64.b64encode(fn_id_str.encode("utf-8"))
